...
# --- [규칙 1] 반드시 최상단 설정 ---
st.set_page_config(page_title="주문 시스템 v6.5", layout="centered")

# --- 0. 데이터 및 영업사원 로드 ---
@st.cache_data
def load_master_data():
    try:
        # dtype=str로 읽어도 엑셀 자체가 float인 경우 .001로 들어오는 경우가 많음
        ...
        def format_code_final(c):
            ...
            if "." in c:
                ...
                suffix = parts[1]
                # [핵심 수정] 뒷자리: 숫자로만 되어있고 4자리가 안 되면 뒤에 0을 채움 (001 -> 0010)
...
# --- 0-1. 영업사원 설정 (박소장님, 장차장님 ID는 찾으시면 꼭 숫자로 바꿔주세요!) ---
...
with st.sidebar.expander("💰 가격 인상 안내", expanded=True):
    ...
    st.info("**2026년 3월 1일부, 일부제품의 평균2.5% 가격 인상이 있습니다. 많은 양해 부탁드립니다.**")
...
for idx, row in f_df.iterrows():
    with st.container(border=True):
        ...
        st.code(row['주문코드']) # 여기서 021.0010으로 정상 출력됨
